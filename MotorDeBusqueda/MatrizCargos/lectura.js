// ============================================================================
// SISTEMA DE AUTOMATIZACIÓN: REQUERIMIENTOS LEGALES SEC. HACIENDA
// Código AppScript para Google Sheets
// ============================================================================
...
function crearTriggerAutomatico() {
  ...
  // Crea trigger de Gmail: cada vez que llega correo a etiqueta específica
  const label = GmailApp.getUserLabelByName("Procesado") || 
                GmailApp.createLabel("Procesado");
  
  // Nota: Gmail no tiene trigger directo, pero podemos usar:
  // - Un trigger de tiempo (cada 5 min busca correos nuevos)
  // - O usar el método manual con onOpen
...
// ============================================================================
// 2. VERIFICACIÓN DE CORREOS NUEVOS (Se ejecuta cada 5 min)
// ============================================================================

function verificaCorreosNuevos() {
  try {
    // Busca correos sin procesar en inbox de notificacionesjudiciales
    const threads = GmailApp.search(
      'from:notificacionesjudiciales@davivienda.co -label:Procesado newer_than:1d'
    );
    
    Logger.log(`📧 Correos nuevos encontrados: ${threads.length}`);
    
    if (threads.length === 0) return; // Sin correos nuevos
    ...
    threads.forEach((thread, idx) => {
      ...
      thread.addLabel(GmailApp.getUserLabelByName("Procesado"));
      thread.removeLabel(GmailApp.getInboxLabel());
    });
...
function crearRecordatorio(idReq, emailTimestamp, diasDespues) {
  try {
    const triggerTime = new Date(emailTimestamp.getTime() + diasDespues * 24 * 60 * 60 * 1000);
    
    // Almacena en propiedades del script para procesar después
  ...
  const ejemplos = [
    {
      ...
      asunto: "Oficio No. PC 0156 del 18 de septiembre de 2024",
...
Medellín, Antioquia`
    },
    {
      ...
      asunto: "Emplazamiento No. 2024-5847",
 ...
 * PASOS PARA INSTALAR:
 * 
 * 1. Abre tu Google Sheet "REQ LEGALES SEC HACIENDA"
 * 2. Tools → Script Editor
 * 3. Copia TODO este código y pega en Apps Script
 * 4. Ejecuta: crearTriggerAutomatico()
 * 5. Cuando pida permisos, acepta
 * 6. Cierra editor
 * 7. Prueba: envía un correo de prueba a notificacionesjudiciales@davivienda.co
 * 8. Espera ~5 minutos (trigger se ejecuta cada 5 min)
 * 9. Revisa Sheets → Nueva fila debería aparecer
 * 10. Tools → Script Editor → Ejecuciones para ver logs
 */